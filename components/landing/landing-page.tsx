...
export interface LandingStats {
  /** Distinct products in the catalog. NOT the row count — 94.4% of rows are
   *  duplicate re-ingests, so the row count overstates it ~18x. */
  toolCount: number
 ...
 * The landing page.
 *
 * Figures arrive as props from the server component in app/page.tsx rather
 * than being fetched on mount. Two reasons: a crawler sees them in the
 * server-rendered HTML, and there is no flash of a missing number. Every value
 * shown here is read from the database — the previous version hard-coded
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                ...
                {toolCount > 0 && (
                  ...
                  // and it is the real distinct-product count, not the row
                  // count the old version used.
                  <div className="inline-flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-primary/80 font-medium">
          ...
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 md:gap-16 items-center">
            ...
            >
              ...
              <p className="text-base sm:text-lg md:text-xl text-muted-foreground mb-4 sm:mb-6">
                Counts come straight from the catalog, refreshed daily. We count
                distinct products, not database rows &mdash; the same
                de-duplication search uses, so the figure matches what you can
                actually find.
              </p>
              <p className="text-sm sm:text-base text-muted-foreground mb-6 sm:mb-8">
                Full public profiles are still being written, so that number is
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 md:gap-6">
              {/* Real figures only.
                *
                * This replaced "50K+ Active Users", "150K+ Daily Searches" and
                * "120+ Countries", all hard-coded and none of them measured —
                * on a site recording roughly one tool view per day. The tool
                * count was real but read the ROW count (272,755) rather than
                * distinct products (15,210), overstating the catalog ~18x.
                *
                * A figure renders only when the database returned it. An
                * em dash is shown otherwise; inventing a placeholder is what
              ...
              {[
                ...
                },
                {
                  label: "Updated",
                  value: "Daily",
                  hint: "new tools ingested every day",
                },
              ].map((stat, i) => (
                <motion.div
                  key={i}
                ...
                >
                  <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-primary mb-1 sm:mb-2 tabular-nums">
                    {stat.value}
                  </div>
                  <div className="text-xs sm:text-sm md:text-base font-medium">{stat.label}</div>
                  {/* What the number actually counts, said plainly. A stat
                    * without its definition is how "7K+ tools" and a directory
                    * of 2,913 ended up on the same site. */}
                  <div className="mt-1 text-[11px] sm:text-xs text-muted-foreground leading-snug">
                    {stat.hint}
                  </div>
                </motion.div>
              ))}
