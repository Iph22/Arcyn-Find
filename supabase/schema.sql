...
-- ============================================
-- This file contains all database tables, indexes, policies, functions, and triggers
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'ai_tools' AND column_name = 'image'
  ) THEN
    ALTER TABLE ai_tools ADD COLUMN image TEXT;
  END IF;
END $$;
...
-- ============================================
-- USER PROFILES (for Clerk authentication)
-- ============================================
CREATE TABLE IF NOT EXISTS user_profiles (
  id TEXT PRIMARY KEY, -- Clerk user IDs are strings like "user_xxxxx"
  username TEXT UNIQUE,
  display_name TEXT,
  avatar_url TEXT,
  banner_url TEXT,
  bio TEXT,
  -- Onboarding and preferences data
  user_role TEXT CHECK (user_role IN ('developer', 'student', 'designer', 'business', 'enthusiast')),
  purpose TEXT,
  experience_level TEXT,
  categories TEXT[] DEFAULT '{}',
  features TEXT[] DEFAULT '{}',
  onboarding_completed BOOLEAN DEFAULT FALSE,
  instructions_seen BOOLEAN DEFAULT FALSE,
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE user_profiles ADD COLUMN user_role TEXT CHECK (user_role IN ('developer', 'student', 'designer', 'business', 'enthusiast'));
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_profiles' AND column_name = 'purpose'
  ) THEN
    ALTER TABLE user_profiles ADD COLUMN purpose TEXT;
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_profiles' AND column_name = 'categories'
  ) THEN
    ALTER TABLE user_profiles ADD COLUMN categories TEXT[] DEFAULT '{}';
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_profiles' AND column_name = 'features'
  ) THEN
    ALTER TABLE user_profiles ADD COLUMN features TEXT[] DEFAULT '{}';
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_profiles' AND column_name = 'instructions_seen'
  ) THEN
    ALTER TABLE user_profiles ADD COLUMN instructions_seen BOOLEAN DEFAULT FALSE;
  END IF;
...
-- Create policies for Clerk (auth handled in API layer with service role key)
...
-- ============================================
-- LEADERBOARDS / STATS VIEW
-- ============================================

-- View: User stats for leaderboards (updated for Clerk)
